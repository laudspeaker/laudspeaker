...
import { ImageBackground, Media, InAppState } from "../types";
...
interface IInAppMediaUploaderProps {
  inAppState: InAppState;
  setInAppState: (inAppState: InAppState) => void;
  currentMainMode: EditorMenuOptions;
}

const InAppMediaUploader = ({
  inAppState,
  setInAppState,
  currentMainMode,
}: IInAppMediaUploaderProps) => {
  const [isMediaLoading, setIsMediaLoading] = useState(false);
  const [progress, setProgress] = useState(0);

  const imageList: { [key: string]: ImageBackground | Media } = {
    [EditorMenuOptions.CANVAS]: inAppState.background.image,
    [EditorMenuOptions.MEDIA]: inAppState.media,
  };
...
export default InAppMediaUploader;
