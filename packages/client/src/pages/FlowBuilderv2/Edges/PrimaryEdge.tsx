...
import { addTemporaryEmptyNodeBetween } from "reducers/flow-builder.reducer";
...
export const PrimaryEdge: FC<EdgeProps> = ({
  ...
  const isTargetEmptyNode =
    nodes.find((node) => node.id === target)?.type === NodeType.EMPTY;

  const isSourceEmptyNode =
    nodes.find((node) => node.id === source)?.type === NodeType.EMPTY;

  return (
    <>
      <BaseEdge path={edgePath} markerEnd={markerEnd} style={style} />
      {!isTargetEmptyNode && !isSourceEmptyNode && isDragging && (
        <EdgeLabelRenderer>
          <div
            style={{
          ...
          >
            <button
              onDragOver={(e) => {
                e.preventDefault();
                dispatch(addTemporaryEmptyNodeBetween({ source, target }));
              }}
            >
              <svg
                width="16"
