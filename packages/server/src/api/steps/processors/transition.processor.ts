...
export class TransitionProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    try {
      switch (job.data.type) {
        ...
        case StepType.START:
          await this.handleStart(job.data.step, job.data.account, job.data.session, queryRunner, transactionSession)
        case StepType.TIME_DELAY:
          await this.handleTimeDelay(job.data.step, job.data.account, job.data.session, queryRunner, transactionSession)
        case StepType.TIME_WINDOW:
          await this.handleTimeWindow(job.data.step, job.data.account, job.data.session, queryRunner, transactionSession)
      ...
      this.error(err, this.process.name, job.data.session, job.data.account.email);
      err = err;
    } finally {
  ...
  async handleABTest(stepID: string, accountID: string, session: string, queryRunner: QueryRunner, transactionSession: mongoose.mongo.ClientSession) {
   ...
   */
  async handleStart(stepID: string, accountID: string, session: string, queryRunner: QueryRunner, transactionSession: mongoose.mongo.ClientSession) {
    ...
    for (let i = 0; i < currentStep.customers.length; i++) {
      nextStep.customers.push(JSON.stringify({ customerID: JSON.parse(currentStep.customers[i]).customerID, timestamp: new Date() }))
    }
    currentStep.customers = [];
    await queryRunner.manager.save(currentStep);
    await queryRunner.manager.save(nextStep);

    this.transitionQueue.add("", { stepID: nextStep.id, accountID: accountID, session: session })
  }
   ...
   * @param transactionSession 
   */
  async handleTimeDelay(stepID: string, accountID: string, session: string, queryRunner: QueryRunner, transactionSession: mongoose.mongo.ClientSession) {
    const currentStep = await queryRunner.manager.findOneBy(Step,
      {
        id: stepID,
        owner: { id: accountID },
    ...
    let forDeletion = []
    for (let i = 0; i < currentStep.customers.length; i++) {
      if (Temporal.Duration.compare(currentStep.metadata.delay, Temporal.Now.instant().since(Temporal.Instant.from(JSON.parse(currentStep.customers[i]).timestamp))) < 0) {
        nextStep.customers.push(JSON.stringify({ customerID: JSON.parse(currentStep.customers[i]).customerID, timestamp: Temporal.Now.instant().toString() }))
        forDeletion.push(currentStep.customers[i]);
      }
    }
    currentStep.customers = currentStep.customers.filter(item => !forDeletion.includes(item));
    await queryRunner.manager.save(currentStep);
    await queryRunner.manager.save(nextStep);
   ...
   * @param transactionSession 
   */
  async handleTimeWindow(stepID: string, accountID: string, session: string, queryRunner: QueryRunner, transactionSession: mongoose.mongo.ClientSession) {
    const currentStep = await queryRunner.manager.findOneBy(Step,
      {
        id: stepID,
        owner: { id: accountID },
  ...
  onActive(job: Job<any, any, any>, prev: string) {
    this.debug(`${JSON.stringify({ job: job })}`, this.onActive.name, job.data.session, job.data.userID);
  }
  ...
  onCompleted(job: Job<any, any, any>, result: any, prev: string) {
    this.debug(`${JSON.stringify({ job: job, result: result })}`, this.onProgress.name, job.data.session, job.data.userID);
  ...
  onProgress(job: Job<any, any, any>, progress: number | object) {
    this.debug(`${JSON.stringify({ job: job, progress: progress })}`, this.onProgress.name, job.data.session, job.data.userID);
  }
  ...
  onStalled(jobId: string, prev: string) {
    this.debug(`${JSON.stringify({ id: jobId, prev: prev })}`, this.onStalled.name, jobId);
  }
}
